...
def _chunked(iterable: Iterable[_T], n: int | None = None) -> Iterator[tuple[_T, ...]]:
...
def set_max_output_height(num_pixels: int) -> None:
  ...
  try:
    # We want to fail gracefully for non-Colab IPython notebooks.
    output = importlib.import_module('google.colab.output')
    s = 'google.colab.output.setIframeHeight(' f'0, true, {{maxHeight: {num_pixels}}})'
    output.eval_js(s)
  except (ModuleNotFoundError, AttributeError):
    pass
...
def _as_valid_media_type(dtype: _DTypeLike) -> _DType:
  ...
  if not issubclass(dtype.type, (np.unsignedinteger, np.floating)):
    raise ValueError(f'Type {dtype} is not a valid media data type (uint or float).')
  return dtype
...
def resize_image(image: _ArrayLike, shape: tuple[int, int]) -> _NDArray:
  ...
  if supported_single_channel or supported_multichannel:
    return np.array(
        _pil_image(image).resize(shape[::-1], resample=PIL.Image.Resampling.LANCZOS),
        dtype=image.dtype,
    )
  if image.ndim == 2:
...
def read_image(
    path_or_url: _Path, *, apply_exif_transpose: bool = True, dtype: _DTypeLike = None
) -> _NDArray:
...
def compress_image(image: _ArrayLike, *, fmt: str = 'png', **kwargs: Any) -> bytes:
...
def _get_video_metadata(path: _Path) -> VideoMetadata:
  ...
  with subprocess.Popen(command, stderr=subprocess.PIPE, encoding='utf-8') as proc:
...
class VideoReader(_VideoIO):
  ...
  def __init__(
    ...
    if output_format not in {'rgb', 'yuv', 'gray'}:
      raise ValueError(f'Output format {output_format} is not rgb, yuv, or gray.')
...
class VideoWriter(_VideoIO):
  ...
  def __init__(
    ...
    if qp is not None and (not isinstance(qp, int) or qp <= 0):
      raise ValueError(f'Quantization parameter {qp} is not a positive integer.')
    ...
    ffmpeg_args = (
        shlex.split(ffmpeg_args) if isinstance(ffmpeg_args, str) else list(ffmpeg_args)
    )
    ...
    if not all_dimensions_are_even and encoded_format.startswith(('yuv42', 'yuvj42')):
      raise ValueError(
...
def show_videos(
  ...
  if isinstance(videos, Mapping):
    if titles is not None:
      raise ValueError('Cannot have both a video dictionary and a titles parameter.')
  ...
  for video, title in zip(list_videos, list_titles):
    ...
    if codec == 'gif':
      ...
      html_string = html_from_compressed_video(data, w, h, title=title, **kwargs)
