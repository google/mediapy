...
_TEST_SHAPES2 = [(128, 128, 3), (128, 160, 1), (160, 128), (64, 64, 3), (64, 64)]
...
class MediapyTest(parameterized.TestCase):
  ...
  def test_pil_image(self):
    im = media._pil_image(np.array([[[10, 11, 12], [40, 41, 42]]], dtype=np.uint8))
  ...
  def test_uint8_to_rgb(self):
    ...
    self.assert_all_close(media.to_rgb(a, vmin=80, vmax=160), gray([0.25, 0.5, 0.75]))
    self.assert_all_close(media.to_rgb(a, vmin=110, vmax=160), gray([0.0, 0.2, 0.6]))
    self.assert_all_close(media.to_rgb(a, vmin=110, vmax=130), gray([0.0, 0.5, 1.0]))
  ...
  def test_show_images_dict(self):
    htmls = []
    with mock.patch('IPython.display.display', htmls.append):
      media.show_images({'title1': media.color_ramp(), 'title2': media.color_ramp()})
    self.assertLen(htmls, 1)
    self.assertIsInstance(htmls[0], IPython.display.HTML)
    self.assertLen(re.findall('(?s)<table', htmls[0].data), 1)
  ...
  def test_video_non_streaming_write_read_roundtrip(self, use_generator):
    ...
    video = (image for image in original_video) if use_generator else original_video
    with tempfile.TemporaryDirectory() as directory_name:
    ...
    with tempfile.TemporaryDirectory() as directory_name:
      ...
      with media.VideoReader(filename1) as reader:
        with media.VideoWriter(
            filename2, reader.shape, metadata=reader.metadata, encoded_format='yuv420p'
        ) as writer:
          for image in reader:
  ...
  def test_video_read_write_10bit(self):
    ...
    fps = 60
    bps = 40_000_000
    horizontal_gray_ramp = media.to_type(np.indices(shape)[1] / shape[1], np.uint16)
    video = np.broadcast_to(horizontal_gray_ramp, (num_images, *shape))
    with tempfile.TemporaryDirectory() as directory_name:
      filename = os.path.join(directory_name, 'test3.mp4')
      media.write_video(filename, video, fps=fps, bps=bps, encoded_format='yuv420p10le')
      new_video = media.read_video(filename, dtype=np.uint16, output_format='gray')
